defmodule Chord do
  ...
  def hello(n,k) do
    ...
    :ets.new(:failure, [:duplicate_bag,:public])
    ...
    final_nodes = Enum.sort(:global.registered_names())
    ...
    new_num_nodes=Enum.count(:global.registered_names())
    #IO.puts("-----------------------------------")
    #IO.puts("new num nodes #{new_num_nodes}")
    denominator = new_num_nodes * numRequests
    avg = total_hops/denominator
    IO.puts("Average hops : #{total_hops} / #{denominator} = #{avg} in #{time_end - time_begin} milliseconds")
    #IO.puts("final registry #{inspect(:global.registered_names())}")
    #IO.puts("initial nodes #{inspect(initial_nodes)}")
    #IO.puts("final nodes #{inspect(final_nodes)}")
    failed_nodes =initial_nodes -- final_nodes
    fail_node_count=Enum.count(failed_nodes)
    IO.puts("failed nodes #{fail_node_count}")
 ...
 def get_keys_between(n1,n2,count,y1) do
